...
- CSS / Tailwind v4 Guidelines:
  - **DO NOT create a `tailwind.config.js` file.**
  - Use CSS variables and `@theme` blocks in `app/assets/tailwind/application.css` for all customizations.
  - This project uses Tailwind v4's modern engine. Conflicting config files will break the build.
- Tests
...
- 3) Context notes
- Context: The project uses Rails 8, Devise, Administrate, Tailwind, and Gemini AI integration via `langchainrb`.
...
6) Color palette
- Respect the color palette passed in assets/tailwind/application.css and never add more colors unless instructed
