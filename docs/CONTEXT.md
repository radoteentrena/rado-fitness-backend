...
* **CSS Framework:** TailwindCSS v4
    * **Note**: No `tailwind.config.js`. Use CSS variables in `app/assets/tailwind/application.css` with `@theme`.
* **Admin Framework:** **Administrate** - *This is the primary UI for the admin.*
...
* Gemfile setup (`administrate`, `devise`, `discard`, `langchainrb`).
* `User` model migration and configuration created.

### 🚧 In Progress: Epic 1 (Architecture) & Epic 2 (Administrate)
1.  **Administrate Dashboard Configuration:** Create dashboards for Users, Exercises.
